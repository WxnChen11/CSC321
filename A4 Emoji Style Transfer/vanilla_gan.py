...
def training_loop(train_dataloader, opts):
    ...
    for epoch in range(opts.num_epochs):
        ...
        for batch in train_dataloader:
            ...
            D_real_loss = float(1)/2/batch_size*torch.sum((D(real_images) - 1) ** 2)
            ...
            D_fake_loss = float(1)/2/batch_size*torch.sum((D(fake_images)) ** 2)
            ...
            G_loss = float(1)/batch_size*torch.sum((D(fake_images) - 1) ** 2)
