...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        D_X_loss = float(1)/len(images_X)*torch.sum((D_X(images_X) - 1) ** 2)
        D_Y_loss = float(1)/len(images_Y)*torch.sum((D_Y(images_Y) - 1) ** 2)
        ...
        D_X_loss = float(1)/len(fake_X)*torch.sum((D_X(fake_X)) ** 2)

        # 4. Generate fake images that look like domain Y based on real images in domain X
        fake_Y = G_XtoY(images_X)

        # 5. Compute the loss for D_Y
        D_Y_loss = float(1)/len(fake_Y)*torch.sum((D_Y(fake_Y)) ** 2)
        ...
        g_loss = float(1)/len(fake_X)*torch.sum((D_X(fake_X)-1) ** 2)

        if opts.use_cycle_consistency_loss:
            reconstructed_Y = G_XtoY(fake_X)
            cycle_consistency_loss = float(1)/len(images_Y)*torch.sum((images_Y - reconstructed_Y) ** 2)
        ...
        g_loss = float(1)/len(fake_Y)*torch.sum((D_Y(fake_Y)-1) ** 2)

        if opts.use_cycle_consistency_loss:
            reconstructed_X = G_YtoX(fake_Y)
            # 3. Compute the cycle consistency loss (the reconstruction loss)
            cycle_consistency_loss = float(1)/len(images_X)*torch.sum((images_X - reconstructed_X) ** 2)
            g_loss += cycle_consistency_loss
