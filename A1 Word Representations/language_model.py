...
class Model(object):
    ...
    def predict_next_word(self, word1, word2, word3, k=10):
        ...
        out = '.'
        for i in idxs[:k]:
            if self.vocab[i] not in punc and out in punc:
                out = self.vocab[i]
...
def train(embedding_dim, num_hid, config=DEFAULT_TRAINING_CONFIG):
    ...
    with open('trained_model.pk', 'wb') as handle:
        cPickle.dump(model, handle, protocol=cPickle.HIGHEST_PROTOCOL)

    return model

def generate_sentence(w1, w2, w3, model):
    a = [w1,w2,w3]
    for i in range(10):
        w = model.predict_next_word(w1, w2, w3)
        a.append(w)
        w1 = w2
        w2 = w3
        w3 = w
...
if __name__ == '__main__':
    if not os.path.isfile('trained_model.pk'):
        model = train(16, 128)
    model = cPickle.load(open('trained_model.pk', 'rb'))
    # w1 = 'when'
    # w2 = 'do'
    # w3 = 'you'
    # generate_sentence(w1, w2, w3, model)
    # w1 = 'you'
    # w2 = 'are'
    # w3 = 'so'
    # generate_sentence(w1, w2, w3, model)
    ...
    w2 = 'a'
    w3 = 'good'
    model.predict_next_word(w1, w2, w3)